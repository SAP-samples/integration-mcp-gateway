...
The following destinations are already configured on the shared tenant and point to publicly hosted mock APIs at [`https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com`](https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com):

| Destination | Type | Path | Represents |
|---|---|---|---|
| `demo_LocationRisk` | OData (default) | `/location-risk` | Location risk data (Everstream mock) |
| `demo_S4_API_MATERIAL_STOCK_SRV` | OData (default) | `/material-stock` | Material stock levels (S/4HANA mock) |
| `demo_S4_sap-s4-ce-plant-0001-v1` | OData (default) | `/plant` | Plant details (S/4HANA mock) |
| `s4hana_api_business-partner` | OData (default) | `/business-partner` | Supplier / Business Partner data |

<details>
<summary>Self-paced setup — expand if you are using your own tenant</summary>


**1. Roles & Entitlements**

Ensure the following are configured on your SAP BTP subaccount:

- A subaccount in **SAP BTP Cloud Foundry environment** with Cloud Foundry enabled
- **SAP Integration Suite** entitled and subscribed
- `Integration_Provisioner` role collection assigned to your user
- **API Composition** activated on the SAP Integration Suite home page
- `Graph.KeyUser` role collection assigned to your user
...
You will create four HTTP destinations in SAP BTP Cockpit — one for each mock API. The steps below walk through creating a single destination; repeat for all four using the names and URLs from the table above.

**Publically hosted URLs**
- [https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/location-risk/](https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/location-risk/)
- [https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/business-partner/](https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/business-partner/)
- [https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/plant/](https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/plant/)
- [https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/plant/material-stock/](https://api-composition-plant-material-risk.cfapps.eu20-002.hana.ondemand.com/plant/material-stock/)

Now create destinations for each of the above urls.

**Step 1 — Navigate to SAP BTP Cockpit**

From SAP Integration Suite, click the **grid icon (⠿)** in the top-right navigation bar and select **SAP BTP Cockpit**.

![SAP Integration Suite — grid icon menu with SAP BTP Cockpit selected](../../resources/screenshots/ex1-prereq-navigate-btp-cockpit.png)

**Step 2 — Open Destinations**

In the BTP Cockpit, go to your subaccount. In the left sidebar, navigate to **Connectivity → Destinations**.

You will see the list of existing destinations. Click **Create**.

![BTP Cockpit — Connectivity → Destinations list with Create highlighted](../../resources/screenshots/ex1-prereq-destinations-list.png)

**Step 3 — Fill in Destination Details**

Fill in the following fields, then click **Create**.

| Field | Value |
|---|---|
| Name | *(destination name from the table above, e.g. `demo_LocationRisk`)* |
| Type | `HTTP` |
...
![Create Destination form — example for demo_LocationRisk](../../resources/screenshots/ex1-prereq-create-destination.png)
...
Repeat steps 2–3 for all four destinations.
